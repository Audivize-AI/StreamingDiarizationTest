...
public class SortformerVectorClustering {
    ...
    public convenience init(
        ...
        fatalError("This initializer needs to be fixed")
        ...
        self.cursorFrame = numFrames
        trimPredictions()
        
        try? self.updateEmbeddingSegments(from: newSegments)
...
//#if DEBUG
//            print("📊 [addChunk IN] cursorFrame=\(cursorFrame), tentativePreds=\(tentativePredictions.count/config.numSpeakers) frames")
//            print("   chunk: newFrames=\(chunk.newFrameCount), oldFrames=\(chunk.oldFrameCount), finalize=\(chunk.finalizedFrameCount)")
//#endif
...
//#if DEBUG
//            let flatSegments = segments.flatMap(\.self)
//            let flatTentative = tentativeSegments.flatMap(\.self)
//            print("📊 [addChunk RESULT] finalized=\(flatSegments.count) segments, tentative=\(flatTentative.count) segments")
//            print("   newSegments count: \(newSegments.count)")
//            for seg in flatSegments {
//                print("   FINAL: speaker \(seg.speakerIndex) [\(seg.startFrame)-\(seg.endFrame)]")
//            }
//            for seg in flatTentative {
//                print("   TENTATIVE: speaker \(seg.speakerIndex) [\(seg.startFrame)-\(seg.endFrame)]")
//            }
//            if !diff.isEmpty {
//                print("   Diff: \(diff.insertions.count) insertions, \(diff.deletions.count) deletions")
...
//#endif
            ...
            print("FINALIZED EMBEDDINGS: \(embeddingSegments.count), TENTATIVE: \(tentativeEmbeddingSegments.count)")
    ...
    private func updateEmbeddingSegments(from segments: [SortformerSegment]) throws {
        ...
        let firstTentativeFrame = cursorFrame - max(config.minUnpaddedGap, minSegmentGap)

        var lastSegment: EmbeddingSegment? = nil
        ...
        for (endFrame, speakerIndex, isStart) in boundaryFrames {
               ...
               let activeSpeaker = activeIds.first
            {
                ...
                if let lastSegment,
                   activeSpeaker == lastSegment.speakerIndex,
                   startFrame - lastSegment.endFrame < minSegmentGap {
                    // Merge with the previous segment
                    newSegment = lastSegment.isFinalized ? embeddingSegments.removeLast() : tentativeEmbeddingSegments.removeLast()
                    newSegment.endFrame = endFrame
                    newSegment.isFinalized = isFinalized
                } else {
                    // Make a new segment
                    newSegment = EmbeddingSegment(
                    ...
                    if let lastSegment {
                        ...
                        lastSegment.addEmbeddings(
                            embeddingManager.takeMatches(for: lastSegment)
                        )
                        
                        // Create embedding requests for the previous segment
                        // Clip to available mel buffer range to avoid requesting dropped frames
                        let requests = lastSegment.getEmbeddingRequests(
                            config: embeddingConfig,
                            availableRange: embeddingManager.availableFrameRange
                        )
                        
                        let embeddings = try embeddingManager.processRequests(requests)
                        lastSegment.addEmbeddings(embeddings)
                    }
                }
                
                // Add the new segment
                if isFinalized {
                    embeddingSegments.append(newSegment)
                } else {
                    tentativeEmbeddingSegments.append(newSegment)
                }
        ...
        if let lastSegment {
            ...
            // We can tolerate errors here for the latest segment as it might be unstable
            if let embeddings = try? embeddingManager.processRequests(requests) {
                lastSegment.addEmbeddings(embeddings)
            }
        }
