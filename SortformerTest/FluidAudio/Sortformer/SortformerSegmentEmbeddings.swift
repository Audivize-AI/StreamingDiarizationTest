...
public class EmbeddingSegment: SpeakerFrameRange, Identifiable {
    ...
    public func getEmbeddingRequests(maxGapSize: Int, maxEmbeddingLength: Int) -> [EmbeddingExtractionRequest] {
        lock.lock()
        ...
        if currentEmbeddings.isEmpty {
            return generateInitialRequests(maxEmbeddingLength: maxEmbeddingLength)
        }
    ...
    private func generateInitialRequests(maxEmbeddingLength: Int) -> [EmbeddingExtractionRequest] {
        var requests: [EmbeddingExtractionRequest] = []
        ...
        // Use overlapping windows for full coverage
        // Stride = 1/2 of maxEmbeddingLength for good overlap
        let stride = max(1, maxEmbeddingLength / 2)
        var currentStart = startFrame
        
        while currentStart < endFrame {
            var currentEnd = min(currentStart + maxEmbeddingLength, endFrame)
            
            // If this would leave a tiny fragment at the end, extend this request
            let remaining = endFrame - currentEnd
            if remaining > 0 && remaining < stride {
                currentEnd = endFrame
            }
...
public struct EmbeddingExtractionRequest {
    /// The segment this embedding is for (direct reference)
    public unowned let segment: EmbeddingSegment
    ...
    /// Add the extracted embedding directly to the segment
    public func addEmbedding(_ embedding: [Float]) {
        let titanetEmbedding = TitaNetEmbedding(
            embedding: embedding,
            startFrame: startFrame,
            endFrame: endFrame
        )
        segment.addEmbeddings([titanetEmbedding])
    }
}
