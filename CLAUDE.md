...
- `DiarizerViewModel.swift` — `@MainActor` ObservableObject. File/mic/enroll flows. Mic tap is installed via a `nonisolated static` helper to avoid Swift 6 isolation-check crashes on the audio RT thread. UI updates from background contexts (tap closure, serial inference queue) go through an `AsyncStream<UIEvent>`: the tap calls `continuation.yield(...)` (nonisolated + `@Sendable`) and a consumer `Task` spawned from `init` drains events on MainActor. Replaces the older `MainActorSink` + `DispatchQueue.main.async` bridge.
...
2. **CoreML output MLMultiArrays use tile-padded strides regardless of `.cpuOnly`.** The mlpackage compiler bakes ANE-friendly memory layout at `compileModel` time; selecting `.cpuOnly` at runtime does **not** switch to tight strides. Observed on LS-EEND dih3: logical shape `[1, 1, 10]` with strides `[16, 16, 1]`. A flat `withUnsafeBufferPointer(ofType: Float.self, Array.init)` returns the physical `T × 16` footprint, which the timeline rejects as `misalignedFinalizedPredictions(16, 10)`. Always read `probs` row-by-row using the published strides (see `LSEENDModel.readProbsStrideAware`), or round-trip through `ANEMemoryUtils.strideAwareCopy` into a dst-allocated dense array. Same rule applies to every state output that feeds back in as input.
3. **`AVAudioEngine` tap closures run on a real-time thread.** Capturing a `@MainActor`-isolated `self` (even weakly) in the tap trips Swift 6's `_swift_task_checkIsolatedSwift` → `dispatch_assert_queue_fail`. Install the tap from a `nonisolated static` helper. The tap must not spawn a `Task { @MainActor ... }` or do any actor hop — it may only call an `AsyncStream<UIEvent>.Continuation.yield(...)` (Sendable + nonisolated), which is drained by a single consumer `Task` spawned from the ViewModel's `@MainActor init`.
...
6. **`fileImporter`'s `isPresented` setter** is called *before* the completion handler — historically a source of ambiguous dispatch. Prefer two separate `.fileImporter(isPresented:)` modifiers, each bound to its own `@State var showXImporter: Bool`, so the completion maps to a single unambiguous action with no shared state to re-read.
...
10. **`MLMultiArray(shape:dataType:)` does not zero its backing buffer.** KV caches and conv caches must be explicitly `memset` to zero in `LSEENDState.init`, or the first chunk reads garbage and emits NaN probs that propagate forever. Any new state tensor added to `LSEENDState` needs the same zeroing.
...
- **RT-thread → MainActor bridge must never be `Task { @MainActor ... }`.** The ViewModel uses an `AsyncStream<UIEvent>` for this: the tap closure calls `continuation.yield(...)` (nonisolated, `@Sendable`) and a single consumer `Task` spawned from the `@MainActor init` applies events. Any refactor that spawns an actor-hopping Task from the tap closure will trip Swift 6's `_swift_task_checkIsolatedSwift` on the audio RT thread.
