...
final class LSEENDRuntimeTests: XCTestCase {
    ...
    private static var cachedEngines: [LSEENDModelVariant: LSEENDInferenceEngine] = [:]
    ...
    private struct RepoFixture {
        let variant: LSEENDModelVariant
    ...
    func testVariantRegistryResolvesAllExportedArtifacts() throws {
        let expectedColumns: [LSEENDModelVariant: Int] = [
            .ami: 4,
            .callhome: 7,
            .dihard2: 10,
            .dihard3: 10,
        ...
        for variant in LSEENDModelVariant.allCases {
    ...
    private func assertOfflineRepoFixtureMatchesSwiftRuntime(variant: LSEENDModelVariant) throws {
    ...
    private func repoFixture(for variant: LSEENDModelVariant) throws -> RepoFixture {
        let metricsURL: URL
        switch variant {
        case .ami:
    ...
    private func makeEngine(variant: LSEENDModelVariant) throws -> LSEENDInferenceEngine {
    ...
    private func runOfflineProbe(variant: LSEENDModelVariant, audioURL: URL) throws -> ProbeInferenceResult {
        try runProbe(
    ...
    private func variantProbeToken(_ variant: LSEENDModelVariant) -> String {
        switch variant {
        case .ami:
            return "ami"
        case .callhome:
